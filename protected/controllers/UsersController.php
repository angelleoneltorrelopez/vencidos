...
'actions'=>array('view','create','update','admin','delete','generarpdf','Excel'),
...
$mPDF1->Output('Reporte - Usuarios '.date('YmdHis').'.pdf','I');  //Nombre del pdf y parámetro para ver pdf o descargarlo directamente.
exit;
}
 ...
 public function actionExcel(){
        ...
        $objPHPExcel->getProperties()->setCreator(Yii::app()->user->name)
             ...
             ->setSubject("Usuarios")
             ->setDescription("Reporte de productos en Usuarios segun la busqueda realizada")
             ->setKeywords("Vencidos, Usuarios, Reporte");
...
$cont = 1;
        ...
        foreach($model as $row):
          ...
          $cont++;
           ...
           $sheet->setCellValue('B'.$cont,$row->username);
           $sheet->setCellValue('C'.$cont,$row->password);
           $sheet->setCellValue('D'.$cont,$row->email);
         endforeach;
