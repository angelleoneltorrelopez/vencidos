...
$session->open();
if(isset($session['ingreso_record']))
//Si hay datos filtrados entonces usar la criteria guardada en la sesion (esto lo guardamos en la funcion search() del modelo)
{
$model=Ingreso::model()->findAll($session['ingreso_record'],array('order'=>'Caducidad ASC, casa.nombrecasa ASC, producto.nombre_productos ASC'));
}
else
...
$mPDF1->SetTitle("Reporte - Ingreso");
...
$mPDF1->Output('Reporte - Ingreso '.date('YmdHis').'.pdf','I');  //Nombre del pdf y parámetro para ver pdf o descargarlo directamente.
exit;
}
