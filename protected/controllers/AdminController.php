...
$session->open();
if(isset($session['Admin_record']))
//Si hay datos filtrados entonces usar la criteria guardada en la sesion (esto lo guardamos en la funcion search() del modelo)
{
$model=Admin::model()->findAll($session['Admin_record'],array('order'=>'Caducidad ASC, casa.nombrecasa ASC, producto.nombre_productos ASC'));
}
else
//Si no hay datos filtrados exportar todo
{
$model =Admin::model()->findAll(array('order'=>'Caducidad ASC, casa.nombrecasa ASC, producto.nombre_productos ASC'));
}
...
$mPDF1->Output('Reporte - Admin '.date('YmdHis'),'I');  //Nombre del pdf y parámetro para ver pdf o descargarlo directamente.
exit;
}
...
public function actionExcel(){
       ...
       $objPHPExcel->getProperties()->setCreator(Yii::app()->user->name)
            ...
            ->setSubject("Admin")
            ->setDescription("Reporte de productos en Admin segun la busqueda realizada")
            ->setKeywords("Vencidos, Admin, Reporte");
...
$cont = 1;
     ...
     // ITEMS
          ...
          $sheet->setCellValue('H'.$cont,$row->Estado);
        endforeach;
